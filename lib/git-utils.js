/**
 * @copyright Copyright 2017, 2023 Kevin Locke <kevin@kevinlocke.name>
 * @license MIT
 */
 ...
 * {@link module:child_process.execFile}.
  ...
  return execFileOut('git', gitArgs, options)
    .then((stdout) => {
      ...
      if (remoteUrl === remoteName) {
        throw new Error(`No URL for ${remoteName} remote`);
      }
      return remoteUrl;
    });
};
...
/** Parses a git URL string into a URL object like {@link module:url.parse} with
...
exports.parseGitUrl = function parseGitUrl(gitUrl) {
  ...
  // Use `git fetch-pack --diag-url $URL` to test git behavior
  const scpParts = /^([^@/]+)@(?:\[([^\]/]+)\]|([^:/]+)):(.*)$/.exec(gitUrl);
  if (scpParts) {
    const [, scpUser, scpBracketedHostPort, scpHostPort, scpPath] = scpParts;
    // Remove square brackets around host unless it is an IPv6 address
    const urlHostPort = scpHostPort ?? (net.isIPv6(scpBracketedHostPort)
      ? `[${scpBracketedHostPort}]`
      : scpBracketedHostPort);
    gitUrl = `ssh://${scpUser}@${urlHostPort}/${scpPath}`;
  }

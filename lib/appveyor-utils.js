 ...
 * @returns {string} AppVeyor build status.
 * @throws {Error} if the status can not be extracted.
 * @private
 */
exports.badgeToStatus = function badgeToStatus(badge) {
  const statuses = [...new Set(badge.match(statusRE))];

  if (statuses.length !== 1) {
    const err = new Error(statuses.length === 0 ? 'Status not found in badge'
 ...
 *
 * @param {!appveyorSwagger.ProjectBuild} projectBuild AppVeyor ProjectBuild
 * (e.g. from getProjectLastBuild).
 * @returns {string} AppVeyor build status.
 * @private
 */
...
exports.parseAppveyorRepoUrl = function parseAppveyorRepoUrl(repoUrl) {
  let repoUrlObj;
  try {
    repoUrlObj = gitUtils.parseGitUrl(repoUrl);
  } catch {
    return {
...
exports.repoUrlToBadgeParams = function repoUrlToBadgeParams(repoUrl) {
  ...
  if (!badgeRepoProviderValues.includes(avRepo.repositoryType)) {
  ...
  if (pathParts.length !== 2) {
    throw new Error('Badge requires repo with 2 path parts.  '
                    + `Found ${pathParts.length}: ${avRepo.repositoryName}`);
  }
 ...
 * @returns {string} Project string in the format {accountName}/{slug}.
 * @private
 */
